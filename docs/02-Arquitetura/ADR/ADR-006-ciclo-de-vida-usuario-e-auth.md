...
> **Nota de implementação (INC-013 G1, 2026-07-23):** o G1 implementa a **fase 1** — anonimização dos dados pessoais após `retentionMonths` (default 24m), preservando o ack sob pseudônimo com o `registration_code` como vínculo mínimo. O `ackRetentionMonths` (default 60m) já existe como campo configurável, mas **a fase 2 — o corte do último vínculo após o prazo prescricional — NÃO foi construída** (TODO): por ora o ack permanece indefinidamente sob pseudônimo (e nunca é deletado). O desenho da fase 2 (severar o `registration_code`? remover a linha do ack, que é imutável?) depende dos prazos definitivos do jurídico (DP-06) e é uma decisão de arquitetura a tomar quando o piloto se aproximar do prazo — não no piloto (ninguém atinge 24m nos primeiros 2 anos).
