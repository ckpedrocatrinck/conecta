...
> **Alinhamento feito (2026-08-05).** A auditoria recebeu nota corretiva datada em três pontos (§6.2, a tabela de reconciliação da §7 e a linha GAP-15 da tabela de gaps) e o INC-023 passou a citar **DP-35** em vez de "GAP-15". O achado original da auditoria não foi reescrito — só anotado. As referências de linha acima (`:533,603`) são de antes dessas inserções; hoje as linhas equivalentes são `:562` e `:639`. A DP-35 é agora a única fonte do deadlock; "GAP-15" volta a significar apenas o item de auditoria.
...
**DP-40 — 🟡 Ausência de barreira TÉCNICA contra `prisma migrate dev` neste projeto (mitigação hoje é 100% documental).** Diagnóstico do INC-027 (investigação do sintoma "senha de demonstração volta para `Trocar123!` a cada reinício") confirmou a causa raiz: o `README.md` recomendava `npx prisma migrate dev` na seção "Banco de dados" — comando que o próprio `ADR-008` já proíbe por escrito para este schema (a coluna `GENERATED` `announcement_versions.search_vector` não é modelável pelo Prisma; `migrate dev` calcula diff contra shadow database, tenta um `ALTER COLUMN ... DROP DEFAULT` que o Postgres recusa, e pode ser interpretado como drift — abrindo caminho para reset do banco + reexecução automática do seed, que recria os usuários de demonstração com senha padrão). Corrigido nesta mesma passada (Bloco 3.11): `README.md` e a seção "Comandos" do próprio `CLAUDE.md` (que também recomendava `migrate dev`, uma segunda ocorrência do mesmo defeito, achada na varredura) agora apontam para `migrate deploy`. **O que permanece em aberto:** `ADR-008` (seção "Consequências") já reconhecia esse risco como aceito conscientemente, mitigado só por documentação ("Risco de regressão se alguém... esquecer este ADR e rodar `prisma migrate dev` direto") — este incidente é a prova de que a mitigação documental sozinha falha na prática: o próprio `README` violou a regra por semanas sem ninguém notar, e nada no código/tooling teria impedido o mesmo comando de rodar contra um banco de staging/produção futuro, se algum dia existir um. **Severidade:** alta se o projeto ganhar um ambiente de staging/produção com dado real antes de uma trava técnica existir (perda de dados sem confirmação); baixa-média hoje, restrita ao banco de dev local. **Opções de trava avaliadas (não implementadas — decisão do Pedro, ver relatório do Bloco 3.11 para prós/contras de cada uma):** script wrapper no `package.json` que force `migrate deploy` e rejeite `migrate dev` explicitamente; checagem em `instrumentation.ts` (roda 1x por processo) recusando o boot se detectar sinal de `migrate dev` em andamento; hook de pre-commit/CI que faz `grep` por `migrate dev` fora do padrão sancionado (`--create-only`) e falha o build; ou nenhuma — manter só documentação, aceitando o risco residual.
