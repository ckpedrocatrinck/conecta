...
**DP-40 — ✅ Resolvida parcialmente (Bloco 5 do INC-027, 2026-08-13): wrapper em `package.json` implementado; cobertura reconhecidamente incompleta, registrada abaixo.** Diagnóstico do INC-027 (investigação do sintoma "senha de demonstração volta para `Trocar123!` a cada reinício") confirmou a causa raiz: o `README.md` recomendava `npx prisma migrate dev` na seção "Banco de dados" — comando que o próprio `ADR-008` já proíbe por escrito para este schema (a coluna `GENERATED` `announcement_versions.search_vector` não é modelável pelo Prisma; `migrate dev` calcula diff contra shadow database, tenta um `ALTER COLUMN ... DROP DEFAULT` que o Postgres recusa, e pode ser interpretado como drift — abrindo caminho para reset do banco + reexecução automática do seed, que recria os usuários de demonstração com senha padrão). Corrigido no Bloco 3.11: `README.md` e a seção "Comandos" do `CLAUDE.md` apontam para `migrate deploy`.

**O que foi implementado (Bloco 5, decisão do Pedro entre as opções avaliadas no Bloco 3.11):** `npm run db:migrate:dev` (`scripts/guard-migrate-dev.js`) recusa rodar e imprime a explicação + o procedimento correto (`migrate deploy` para aplicar; `migrate dev --create-only` + edição manual do ADR-008 para criar migration nova), apontando para `ADR-008`. Testado: `npm run db:migrate:dev` sai com código 1 e a mensagem esperada, sem tentar nada contra o banco. `npm run db:migrate` (alias documentado para `prisma migrate deploy`) testado e funcional.
