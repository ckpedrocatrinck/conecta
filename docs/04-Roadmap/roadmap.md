...
| 025 | Serviço `app` no Docker Compose (dev/local): `output: "standalone"`, Dockerfile, `.dockerignore`, serviços `app` e `migrate` | infra (pré-piloto) | 🟡 código completo (2026-08-06) — critérios (a) health, (b) agendador `OK 200` e (c) `migrate deploy` no container verificados; falta só o (d), que depende de `NEXT_PUBLIC_VAPID_PUBLIC_KEY` no `.env`. Não mergeado: Pedro revisa antes. Pipeline CI→GHCR→VPS do ADR-011 §9 fora de escopo | ADR-011 · 023 |
