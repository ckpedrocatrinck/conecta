...
| 025 | Serviço `app` no Docker Compose (dev/local) | infra (pré-piloto) | 🟡 código completo (2026-08-06), branch `inc-025-servico-app-compose` — **implementado, não validado, não mergeado na main**: falta o critério (d) (chaves VAPID reais no `.env` + push confirmado através do container, nunca exercitado no projeto) e revisão do Pedro (primeiro build de produção via Docker aqui) | ADR-011 (§8), 023 |
| 026 | Handler de push no service worker | correção (pré-piloto) | 🟡 código completo (2026-08-06), branch `inc-026-push-service-worker` (empilhada sobre a 025, não sobre a main) — **implementado, não validado, não mergeado**: pendente de teste real de push em iPhone (critérios a/b) e do merge prévio do 025 | 012, 025 |
| 027 | Preparação e publicação do repositório (higiene de repositório / infraestrutura de portfólio) | fora de fase | ✅ (2026-08-14) Blocos 0–6 concluídos — repositório público em https://github.com/ckpedrocatrinck/conecta; duas exceções registradas (repositório antigo não deletado por perda de acesso; bundle cifrado mantido como única cópia do histórico original) | ADR-012 aceito |
