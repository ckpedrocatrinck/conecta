...
| 024 | Correções de navegação e fluxo (Módulo B): link de aniversariantes, `error.tsx` do tenant, card de vaga no feed, foto do post | correção (pré-piloto) | 🟡 Partes 1 e 2 entregues (2026-08-05, resolve DP-30) — Partes 3 (card de vaga no feed) e 4 (foto do post, é a DP-19) paradas na investigação, aguardando decisão do Pedro | 008, 010, 011 |
