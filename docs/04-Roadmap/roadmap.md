...
| 023 | Correções críticas do núcleo jurídico (agendador de cron no Compose; reabertura de pendência por mudança material verificada como já funcionando) | correção (pré-piloto) | ✅ (2026-08-06) — validação end-to-end do agendador feita no INC-025 (2 ciclos `OK 200`, zero `FALHA`) | 004, 005 · ADR-011 |
...
| 025 | Serviço `app` no Docker Compose (dev/local): `output: "standalone"`, Dockerfile, `.dockerignore`, serviços `app` e `migrate` | infra (pré-piloto) | 🟡 código completo (2026-08-06) — critérios (a) health, (b) agendador `OK 200` e (c) `migrate deploy` no container verificados; o (d) segue **pendente** — depende das chaves VAPID reais no `.env` + push confirmado através do container (gap pré-existente do INC-012, não introduzido aqui). Não mergeado: aguarda sinal verde do Pedro. Pipeline CI→GHCR→VPS do ADR-011 §9 fora de escopo | ADR-011 · 023 |
| 026 | Handler de `push`/`notificationclick` no service worker (nunca existiu — fecha a lacuna de medição real que mantém o INC-012 em 🟡) + fim do catch mudo em `push-channel.ts` | correção (pré-piloto) | 🟡 código completo (2026-08-06) — aguarda o teste no iPhone real do Pedro (critérios (a) exibir com app fechado e (b) clique abrir/focar). Branch empilhada sobre a do INC-025; merge depende dela | 012 · 025 |
