...
**Node.js 20 depreciado.** Annotation do run também acusava `actions/checkout@v4`/`actions/setup-node@v4` forçadas a Node 24 (runtime das próprias actions já não suportado). Atualizadas para `@v7` (última major de ambas, já roda nativamente em Node 24; sem breaking change relevante para este workflow — `v7` do `checkout` só muda o comportamento default de `pull_request_target`/`workflow_run`, que este workflow não usa, e o `v7` do `setup-node` só muda detecção automática de cache para outros gerenciadores além do npm) — `239412b`.

**Verificação real, não só local:** as duas correções foram mergeadas (`d024a5c`, `--no-ff`) e o push disparou um novo run em `main` (`31801861681`) — verde, os 9 steps de `quality` passando, sem a annotation de Node 20. `npm run lint && typecheck && test && build` também verdes localmente antes do commit (360/360 testes).
