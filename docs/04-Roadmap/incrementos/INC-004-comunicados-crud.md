...
- **Requisito operacional para migração de dados históricos**: qualquer processo que crie `Announcement` com `seq_number` pré-atribuído fora de `publishAnnouncement()` (ex.: importação de numeração legada da portal legado) precisa também atualizar `announcement_sequences.last_number` para o máximo usado naquele tenant+ano — documentado no comentário de `prisma/seed-data.ts`, mas vale um lembrete formal quando a migração de dados legados for planejada.
