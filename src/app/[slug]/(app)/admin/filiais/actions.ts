...
export async function createBranchAction(formData: FormData) {
  ...
  if (!name || !code) redirect(`/${session.tenantSlug}/admin/filiais?erro=obrigatorio`);
  ...
  if (outcome === "duplicado") redirect(`/${session.tenantSlug}/admin/filiais?erro=duplicado`);
  redirect(`/${session.tenantSlug}/admin/filiais?ok=criada`);
}
...
export async function updateBranchAction(formData: FormData) {
  ...
  const name = String(formData.get("name") ?? "").trim();
  const code = String(formData.get("code") ?? "").trim();
  if (!id || !name || !code) redirect(`/${session.tenantSlug}/admin/filiais?erro=obrigatorio`);
  ...
  redirect(`/${session.tenantSlug}/admin/filiais?ok=atualizada`);
}

export async function deleteBranchAction(formData: FormData) {
  const session = await requireAdmin();
  const id = String(formData.get("id") ?? "");
  if (!id) redirect(`/${session.tenantSlug}/admin/filiais`);
  ...
  if (outcome === "em-uso") redirect(`/${session.tenantSlug}/admin/filiais?erro=em-uso`);
  redirect(`/${session.tenantSlug}/admin/filiais?ok=removida`);
}
