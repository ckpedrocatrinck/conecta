...
export function EditAnnouncementForm({
  ...
  if (announcement.status === "archived") {
    return (
      <div className="flex flex-col gap-3 rounded-[var(--radius-card)] border border-border bg-card p-4 shadow-[var(--shadow-card)]">
        <p className="text-meta text-muted-foreground">Comunicado arquivado — sem edição possível.</p>
        <h2 className="text-card-title font-bold text-foreground">{latest?.title}</h2>
  ...
  return (
    <div className="flex flex-col gap-6">
      <form action={saveAnnouncementContentAction} className="flex w-full max-w-2xl flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {criticalityLocked && (
            <p className="text-meta text-muted-foreground">Já publicado — criticidade não pode mais ser alterada.</p>
          )}
        </div>
      ...
      <div className="flex flex-wrap items-end gap-3 border-t border-border pt-4">
        {(announcement.status === "draft" || announcement.status === "scheduled") && (
          <form action={publishAnnouncementNowAction}>
            <input type="hidden" name="id" value={announcement.id} />
            <SubmitButton size="touch" pendingLabel="Publicando…">Publicar agora</SubmitButton>
          </form>
        )}
        ...
        {announcement.status === "draft" && (
          <form action={scheduleAnnouncementAction} className="flex items-end gap-2">
            ...
            <SubmitButton variant="secondary" size="touch" pendingLabel="Agendando…">
              Agendar
            </SubmitButton>
          </form>
        )}
        ...
        {announcement.status === "scheduled" && (
          <>
            ...
            <form action={unscheduleAnnouncementAction}>
              <input type="hidden" name="id" value={announcement.id} />
              <SubmitButton variant="secondary" size="touch" pendingLabel="Cancelando…">
