...
export async function updateEmployeeAction(formData: FormData) {
  ...
  if (!id || !fullName || !branchId || !VALID_ROLES.has(role)) {
    redirect(`/${session.tenantSlug}/admin/colaboradores/${id}?erro=obrigatorio`);
  }
  ...
  if (outcome === "filial-invalida") redirect(`/${session.tenantSlug}/admin/colaboradores/${id}?erro=filial`);
  redirect(`/${session.tenantSlug}/admin/colaboradores/${id}?sucesso=1`);
}
...
export async function toggleEmployeeStatusAction(formData: FormData) {
  ...
  if (!id) redirect(`/${session.tenantSlug}/admin/colaboradores`);
  ...
  redirect(`/${session.tenantSlug}/admin/colaboradores/${id}?status=${nextStatus === "active" ? "reativado" : "desligado"}`);
}
