    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-5 px-4 py-6">
      ...
      {branches.length > 1 && (
        <div className="flex flex-wrap gap-2">
          <Link href={`/${session.tenantSlug}/vagas`} className={filterChipVariants({ active: !filial })}>
            Todas as filiais
          </Link>
          {branches.map((b) => (
            <Link
              key={b.id}
              href={{ pathname: `/${session.tenantSlug}/vagas`, query: { filial: b.id } }}
        ...
        <div className="flex flex-col gap-3">
          {jobs.map((job) => (
            <div key={job.id} className="flex flex-col gap-2">
              <Link href={`/${session.tenantSlug}/vagas/${job.id}`}>
