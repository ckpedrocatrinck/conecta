...
export default async function ComunicadosColaboradorPage({
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-5 px-4 py-6">
      ...
      <form className="flex flex-col gap-3" action={`/${session.tenantSlug}/comunicados`}>
        ...
        {categories.length > 0 && (
          <div className="flex flex-wrap gap-2">
            <Link
              href={{ pathname: `/${session.tenantSlug}/comunicados`, query: q ? { q } : {} }}
            ...
            {categories.map((c) => (
              <Link
                key={c}
                href={{ pathname: `/${session.tenantSlug}/comunicados`, query: { ...(q ? { q } : {}), categoria: c } }}
